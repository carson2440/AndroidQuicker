...
public class QByteUtil {
     ...
     *
     * @param byteArray
     * @param sperator  输出字符串之间的间隔符号
     * @return
     */
    public static String toHexString(byte[] byteArray, String sperator) {
        String result = "";
        if (QStrings.isNotEmpty(byteArray)) {
            StringBuilder builder = new StringBuilder();
            for (int i = 0; i < byteArray.length; i++) {
                builder.append(Integer.toString((byteArray[i] & 0xff) + 0x100, 16).substring(1));
                if (QStrings.isNotEmpty(sperator)) {
    ...
    public static byte[] toByteArray(String hexString) {
        ByteBuffer bf = ByteBuffer.allocate(hexString.length() / 2);
     ...
     *
     * @param bytes
     * @param lowAndHigh 是否是低位在前，高位在后
     * @return
     */
    public static String getCRC16(byte[] bytes, boolean lowAndHigh) {
        int crc = 0x0000ffff;
        int ucCRCHi = 0x00ff;
        int ucCRCLo = 0x00ff;
        int iIndex;
